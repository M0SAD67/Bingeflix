...
async function extractStreamUrl(url) {
    ...
    try {
        ...
        const data1 = await soraFetch(embedUrl, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
                'Referer': 'https://vidsrc.su/'
            }
        }).then(res => res.text());
        console.log('Embed HTML length:', data1.length);

                const urlRegex = /url\s*:\s*(['"])(.*?)\1/gm;
                ...
                console.log('Streams found:', streams2);
        ...
        console.log('Embed HTML length:', data1.length);
                ...
                let subtitle = '';
                // const engMatch = Array.from(data1.matchAll(subtitleRegex)).find(([, url,, display]) => display.includes('Arabic'));
                ...
                //     const subtitleTrack = subtitleTrackData.find(track =>
                //         track.display.startsWith('Arabic')
                //     );
